...
import logging
from  .base import _DATA_REGISTRY
...
def data_action(action: str):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def register_action(cls):
            logger.info(f"Data Action Internal Register {action} for class {cls.__name__}")
            nonlocal cls_name
            cls_name = cls.__name__
            if cls_name not in _DATA_REGISTRY:
                _DATA_REGISTRY[cls_name] = {}
            _DATA_REGISTRY[cls_name][action] = metadata
            return cls
        
        setattr(wrapper, 'data_register', register_action)
