...
import logging
from  .base import BaseOperator,_DEVICE_NAME_REGISTRY,_ACTION_REGISTRY
...
def device_action(action: str):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def register_action(cls):
            logger.info(f"Device Action Internal Register {action} for class {cls.__name__}")
            nonlocal cls_name
            cls_name = cls.__name__
            if cls_name not in _ACTION_REGISTRY:
                _ACTION_REGISTRY[cls_name] = {}
            _ACTION_REGISTRY[cls_name][action] = metadata
            return cls
        
        setattr(wrapper, 'device_register', register_action)
